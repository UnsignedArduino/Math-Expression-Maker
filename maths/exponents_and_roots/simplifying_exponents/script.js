...
function generate_expression_part(types, min, max, chance_to_expr, decimal_places) {
  if (random_number(0, 1) < chance_to_expr) {
    ...
    if (randomOp === ExponentationOperation) {
      return new randomOp(
...
function generate_equation() {
  ...
  let equation_t = math.parse(equation).toTex({parenthesis: "auto", implicit: "hide"});
  console.log("TeX: "  + equation_t);
  let answer = math.simplify(equation);
  console.log("Answer: " + answer);
  let answer_t = answer.toTex({parenthesis: "auto", });
